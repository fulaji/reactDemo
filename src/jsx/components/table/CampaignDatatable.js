    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire('campaign Deleted Successfully!', '', 'success');
        } catch (err) {
          ...
          <div className="table-responsive">
            <div id="job_data" className="dataTables_wrapper ">
              ...
              >
                ...
                <tbody>
                  {data.length === 0 && <tr className="odd" role="row"><td className="sorting_1" colSpan="12">No Campaign found!</td></tr>}
                  {data.map((property, i) => (
                    <tr className="odd" role="row" key={i}>
                      ...
                      {/* <td>
                        <Link to={`/campaign/${property.id}`}>
                            <Highlighter
                              highlightClassName="YourHighlightClass"
                              searchWords={[searchKey]}
                              autoEscape={true}
                              textToHighlight={`${property.title}`}
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={formatForecastingStage(property.forward_number)}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      {/* <Link to={`/property-details/${property.id}`}> */}
                        <td className="sorting_1">
                        {property.text?.length >= 15
                            ? ( <OverlayTrigger
                              trigger={['hover', 'focus']}
                              placement="right"
                              overlay={(
                            ...
                            >
                              <span >{property.text.substring(0,15)+'...'}</span> 
                            </OverlayTrigger>)
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={property.called}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={property.answered}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={property.pressed1}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={property.total_cost}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      ...
                      <td>
                        <Highlighter
                          ...
                          searchWords={[searchKey]}
                          autoEscape={true}
                          textToHighlight={property.created_at}
                          highlightStyle={{ background: "#ff0" }}
                        />
                      </td>
                      <td className="d-flex justify-content-center align-items-center">
                        ...
                        <button className="btn btn-danger rounded ml-1"  onClick={() => handleDeleteProperty(property)}>Delete</button>
                      </td>
...
const mapStateToProps = (state) => ({
  campaigns: state.campaigns.messages,
  totalMessages: state.campaigns.totalMessages,
  activePage: state.campaigns.activePage,
  limit: state.campaigns.limit,
  tableHeaders: state.campaigns.tableHeaders,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(CampaignDatatable);